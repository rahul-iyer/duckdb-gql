...
#encoding: utf-8
#
# Source: https://github.com/opencypher/openCypher/blob/677cbafabb8c3c5eed458fd3b1ec0daec8d67d23/tck/features/clauses/with/With5.feature
...
Feature: With5 - Implicit grouping with DISTINCT
  ...
  Scenario: [2] Handling DISTINCT with lists in maps
    ...
    And having executed:
      """
      INSERT ({list: ['A', 'B']}), ({list: ['A', 'B']})
      """
    When executing query:
      """
      MATCH (n)
      LET __gql_with_scope_1 = 1, map = {name: n.list}
      RETURN count(*)
      """
    Then the result should be, in any order:
      | count(*) |
