...
#encoding: utf-8
#
# Source: https://github.com/opencypher/openCypher/blob/677cbafabb8c3c5eed458fd3b1ec0daec8d67d23/tck/features/clauses/with-where/WithWhere5.feature
...
Feature: WithWhere5 - Filter on predicate resulting in null
  ...
  Scenario: [1] Filter out on null
    ...
    And having executed:
      """
      INSERT (root:Root {name: 'x'}),
             (child1:TextNode {var: 'text'}),
             (child2:IntNode {var: 0})
      INSERT (root)-[:T]->(child1),
             (root)-[:T]->(child2)
      """
    When executing query:
      """
      MATCH (:Root {name: 'x'})-->(i:TextNode)
      LET __gql_with_scope_1 = 1, __gql_with_1_1_i = i
      WHERE i.var > 'te'
      RETURN i
      """
