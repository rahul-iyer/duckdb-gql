...
#encoding: utf-8
#
# Source: https://github.com/opencypher/openCypher/blob/677cbafabb8c3c5eed458fd3b1ec0daec8d67d23/tck/features/clauses/match-where/MatchWhere6.feature
...
Feature: MatchWhere6 - Filter optional matches
  ...
  Scenario: [1] Filter node with node label predicate on multi variables with multiple bindings after MATCH and OPTIONAL MATCH
    ...
    And having executed:
      """
      INSERT (a {name: 'A'}), (b:B {name: 'B'}), (c:C {name: 'C'}), (d:D {name: 'C'})
      INSERT (a)-[:T]->(b),
             (a)-[:T]->(c),
             (a)-[:T]->(d)
      """
