...
#encoding: utf-8
#
# Source: https://github.com/opencypher/openCypher/blob/677cbafabb8c3c5eed458fd3b1ec0daec8d67d23/tck/features/clauses/return/Return6.feature
...
Feature: Return6 - Implicit grouping with aggregates
  ...
  Scenario: [3] Aggregating by a list property has a correct definition of equality
    ...
    And having executed:
      """
      INSERT ({a: [1, 2, 3]}), ({a: [1, 2, 3]})
      """
    When executing query:
      """
      MATCH (a)
      LET __gql_with_scope_1 = 1, a = a.num, count = count(*)
      RETURN count
      """
    Then the result should be, in any order:
      | count |
  ...
  Scenario: [13] Returning the minimum length of paths
    ...
    And having executed:
      """
      INSERT (a:T {name: 'a'}), (b:T {name: 'b'}), (c:T {name: 'c'})
      INSERT (a)-[:R]->(b)
      INSERT (a)-[:R]->(c)
      INSERT (c)-[:R]->(b)
      """
    When executing query:
      ...
      WHERE other <> a
      LET __gql_with_scope_1 = 1, __gql_with_1_1_a = a, __gql_with_1_2_other = other, len = min(length(p))
  ...
  Scenario: [16] Aggregation on complex expressions
    ...
    And having executed:
      """
      INSERT (andres {name: 'Andres'}),
             (michael {name: 'Michael'}),
             (peter {name: 'Peter'}),
             (bread {type: 'Bread'}),
             (veggies {type: 'Veggies'}),
             (meat {type: 'Meat'})
      INSERT (andres)-[:ATE {times: 10}]->(bread),
    ...
    When executing query:
      ...
      LET __gql_with_scope_1 = 1, __gql_with_1_1_me = me, H1 = count(DISTINCT r1), H2 = count(DISTINCT r2), __gql_with_1_4_you = you
      MATCH (me)-[r1:ATE]->()<-[r2:ATE]-(you)
