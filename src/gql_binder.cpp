...
static bool IsNumeric(GqlTypeId type) {
  return type == GqlTypeId::INTEGER || type == GqlTypeId::ELEMENT_ID ||
         type == GqlTypeId::DECIMAL || type == GqlTypeId::DOUBLE ||
         type == GqlTypeId::PROPERTY_VALUE;
}
...
static bool IsScalar(GqlTypeId type) {
  return type != GqlTypeId::UNKNOWN && type != GqlTypeId::NODE &&
         type != GqlTypeId::EDGE && type != GqlTypeId::PATH;
}
...
static bool ExpressionsEqual(const GqlBoundExpression &left,
                             const GqlBoundExpression &right) {
  if (left.expression_type != right.expression_type ||
      ...
      left.unary_operator != right.unary_operator ||
      left.binary_operator != right.binary_operator ||
      left.function_name != right.function_name ||
      left.aggregate != right.aggregate || left.distinct != right.distinct ||
...
static GqlType NumericResult(const GqlType &left, const GqlType &right) {
  GqlType result;
  if (left.id == GqlTypeId::PROPERTY_VALUE ||
      right.id == GqlTypeId::PROPERTY_VALUE) {
...
void GqlBinder::PushScope() {
  if (current_scope == DConstants::INVALID_INDEX ||
      current_scope >= scopes.size()) {
...
idx_t GqlBinder::DefineBinding(GqlBinding binding) {
  ...
  if (!scopes[current_scope]
           .bindings.emplace(binding.name, binding_index)
           .second) {
    throw BinderException("GQL binding variable '%s' is already defined",
                          binding.name);
  }
...
idx_t GqlBinder::ResolveIndex(const GqlIdentifier &identifier) const {
  ...
  if (binding_index == DConstants::INVALID_INDEX) {
    throw BinderException(
        "GQL binding variable '%s' is not defined (line %llu, column "
        "%llu)",
        identifier.value,
...
const GqlExpression &
GqlBinder::ResolveValueExpression(const GqlExpression &expression) const {
  ...
  for (idx_t field_index = 0; field_index < receiver.field_names.size();
       field_index++) {
  ...
  throw BinderException("GQL LET record field '%s' does not exist",
...
shared_ptr<GqlBoundExpression>
GqlBinder::BindExpression(const GqlExpression &expression) {
  auto &resolved = ResolveValueExpression(expression);
  ...
  case GqlExpressionType::VARIABLE_REFERENCE: {
    ...
    if (binding.type.id == GqlTypeId::PATH) {
      ...
      for (const auto &element : path->second) {
        ...
        argument->result_type = {element.type == GqlPatternElementType::VERTEX
                                     ? GqlTypeId::NODE
                                     : GqlTypeId::EDGE,
  ...
  case GqlExpressionType::FUNCTION: {
    for (const auto &argument : expression.arguments) {
      ...
      if (expression.aggregate && ContainsAggregate(*bound)) {
        throw BinderException(
            "Nested GQL aggregate functions are not supported");
      }
    ...
    if (name == "char_length" || name == "length") {
      ...
      result->result_type = {GqlTypeId::INTEGER,
                             result->arguments[0]->result_type.nullable};
      return result;
    }
    if (name == "abs" || name == "ceil" || name == "floor" || name == "sqrt" ||
        name == "mod") {
      if (!IsNumeric(result->arguments[0]->result_type.id)) {
        throw BinderException("GQL function '%s' requires numeric arguments",
                              expression.function_name);
      }
      if (name == "mod" && (result->arguments.size() != 2 ||
                            !IsNumeric(result->arguments[1]->result_type.id))) {
        throw BinderException(
            "GQL function MOD requires two numeric arguments");
      }
    ...
    if (name == "sum" || name == "avg") {
      if (!IsNumeric(result->arguments[0]->result_type.id)) {
        throw BinderException("GQL aggregate '%s' requires a numeric argument",
                              expression.function_name);
      }
      result->result_type = result->arguments[0]->result_type;
    ...
    if (name == "min" || name == "max") {
      if (!IsScalar(result->arguments[0]->result_type.id)) {
        throw BinderException("GQL aggregate '%s' requires a scalar argument",
                              expression.function_name);
      }
      result->result_type = result->arguments[0]->result_type;
      result->result_type.nullable = true;
    ...
    throw NotImplementedException("GQL function '%s'",
                                  expression.function_name);
  }
  case GqlExpressionType::UNARY:
    ...
    if (expression.unary_operator == GqlUnaryOperator::NOT) {
      ...
      result->result_type = {GqlTypeId::BOOLEAN,
                             result->left->result_type.nullable};
    } else {
  ...
  case GqlExpressionType::BINARY:
    ...
    case GqlBinaryOperator::SUBTRACT:
      if (!IsNumeric(result->left->result_type.id) ||
          !IsNumeric(result->right->result_type.id)) {
        throw BinderException("GQL arithmetic operands must be numeric");
      }
      result->result_type =
          NumericResult(result->left->result_type, result->right->result_type);
      break;
    ...
    case GqlBinaryOperator::GREATER_THAN_OR_EQUAL:
      if (!IsScalar(result->left->result_type.id) ||
          !IsScalar(result->right->result_type.id)) {
...
string GqlBinder::ProjectionName(const GqlProjection &projection,
                                 const GqlBoundExpression &expression) const {
  ...
  if (expression.expression_type == GqlExpressionType::PROPERTY_REFERENCE &&
      binding) {
...
vector<GqlLogicalPlan>
GqlBinder::BindAlternatives(const GqlMatchStatement &statement) {
  vector<pair<idx_t, idx_t>> ranged_elements;
  vector<pair<idx_t, idx_t>> any_direction_elements;
  idx_t alternative_count = 1;
  for (idx_t pattern_index = 0; pattern_index < statement.patterns.size();
       pattern_index++) {
    const auto &pattern = statement.patterns[pattern_index];
    for (idx_t element_index = 0; element_index < pattern.elements.size();
         element_index++) {
  ...
  for (idx_t alternative = 0; alternative < alternative_count; alternative++) {
    ...
    for (const auto &location : any_direction_elements) {
      auto &element = branch.patterns[location.first].elements[location.second];
      element.edge_direction =
          selection % 2 == 0 ? GqlEdgeDirection::RIGHT : GqlEdgeDirection::LEFT;
      selection /= 2;
    }
...
GqlLogicalPlan GqlBinder::Bind(const GqlMatchStatement &statement) {
  ...
  auto bind_pattern = [&](const GqlPattern &pattern) {
    ...
    auto bind_element = [&](const GqlPatternElement &element,
      ...
      if (force_anonymous || element.variable.IsEmpty()) {
        ...
        if (existing != DConstants::INVALID_INDEX) {
          ...
          auto expected_type = element.type == GqlPatternElementType::VERTEX
                                   ? GqlTypeId::NODE
                                   : GqlTypeId::EDGE;
          ...
          binding.type = {element.type == GqlPatternElementType::VERTEX
                              ? GqlTypeId::NODE
                              : GqlTypeId::EDGE,
                          false};
      ...
      if (element.unbounded) {
        if (element.type != GqlPatternElementType::EDGE ||
          ...
          throw InternalException(
              "Invalid unbounded quantified GQL path element");
        }
        bind_element(element, true);
        continue;
      }
      if (element.type != GqlPatternElementType::EDGE ||
          element.minimum_repetitions == 0 ||
          element.minimum_repetitions != element.maximum_repetitions ||
          !element.variable.IsEmpty()) {
        throw InternalException("Invalid fixed quantified GQL path element");
      }
      for (idx_t repetition = 0; repetition < element.minimum_repetitions;
           repetition++) {
        bind_element(element, true);
    ...
    if (!pattern.variable.IsEmpty()) {
      if (FindBindingIndex(pattern.variable.value) !=
  ...
  auto bind_predicate = [&](idx_t predicate_index) {
    if (predicate_index >= statement.predicates.size()) {
      throw InternalException(
          "GQL clause predicate index is outside the AST predicate pool");
    }
    ...
    if (ContainsAggregate(*bound_predicate)) {
      throw BinderException(
          "GQL aggregate functions are not allowed in WHERE or FILTER");
    }
  ...
  for (const auto &clause : statement.clauses) {
    ...
    case GqlQueryClauseType::MATCH: {
      if (saw_return_clause || saw_call_clause || clause.pattern_count == 0 ||
          ...
          clause.pattern_count >
              statement.patterns.size() - clause.pattern_begin) {
      ...
      for (idx_t pattern_index = clause.pattern_begin;
           pattern_index < clause.pattern_begin + clause.pattern_count;
           pattern_index++) {
        if (bound_patterns[pattern_index]) {
          throw InternalException(
              "GQL pattern belongs to more than one MATCH clause");
        }
        const auto &pattern = statement.patterns[pattern_index];
        if (pattern.optional != clause.optional ||
            pattern.optional_stage != clause.optional_stage) {
          throw InternalException(
              "GQL MATCH clause optionality disagrees with its pattern");
        }
      ...
      for (auto predicate_index : clause.predicate_indices) {
        if (predicate_index >= bound_predicate_indices.size() ||
            bound_predicate_indices[predicate_index]) {
          throw InternalException("Invalid GQL MATCH predicate ownership");
        }
        auto filter =
            make_shared_ptr<GqlLogicalFilter>(bind_predicate(predicate_index));
      ...
      if (clause.optional) {
        auto apply =
            make_shared_ptr<GqlLogicalLeftApply>(clause.optional_stage);
        apply->child =
            current ? std::move(current) : make_shared_ptr<GqlLogicalUnit>();
    ...
    case GqlQueryClauseType::FILTER: {
      ...
      auto filter = make_shared_ptr<GqlLogicalFilter>(
          bind_predicate(clause.predicate_index));
      if (active_optional_apply &&
          active_optional_apply->optional_stage == clause.optional_stage) {
        ...
        throw InternalException(
            "GQL FILTER has no active OPTIONAL MATCH stage");
      } else {
    ...
    case GqlQueryClauseType::LET: {
      ...
      for (idx_t let_index = clause.let_begin;
           let_index < clause.let_begin + clause.let_count; let_index++) {
        auto &binding = statement.let_bindings[let_index];
        if (!binding.expression) {
          throw InternalException("GQL LET binding has no expression");
        }
        if (FindBindingIndex(binding.variable.value) !=
                DConstants::INVALID_INDEX ||
            let_bindings.find(binding.variable.value) != let_bindings.end()) {
          throw BinderException("GQL LET variable '%s' is already defined",
    ...
    case GqlQueryClauseType::CALL: {
      ...
      auto definition = GqlFindProcedure(call.procedure_namespace.value,
                                         call.procedure_name.value);
      if (!definition) {
        throw BinderException("Unknown GQL procedure '%s.%s'",
                              call.procedure_namespace.value,
                              call.procedure_name.value);
      }
      ...
      if (call.arguments.size() < required_arguments ||
          call.arguments.size() > definition->arguments.size()) {
        throw BinderException(
            "GQL procedure '%s.%s' expects between %llu and %llu arguments, "
            "got %llu",
      ...
      for (idx_t argument_index = 0; argument_index < call.arguments.size();
           argument_index++) {
        ...
        if (argument.mode == GqlProcedureArgumentMode::CONFIGURATION) {
          if (expression->type != GqlExpressionType::LITERAL) {
            throw BinderException(
                "GQL procedure configuration argument '%s' must be constant",
                argument.name);
          }
          auto type = LiteralType(expression->literal);
          if (type.id != argument.type.id) {
            throw BinderException(
                "GQL procedure configuration argument '%s' has an invalid type",
                argument.name);
          }
        ...
        if (ContainsAggregate(*bound)) {
          throw BinderException(
              "GQL procedure input argument '%s' cannot contain an aggregate",
              argument.name);
        }
        if (bound->result_type.id != argument.type.id &&
            !(argument.type.id == GqlTypeId::ELEMENT_ID &&
              bound->result_type.id == GqlTypeId::INTEGER)) {
          throw BinderException(
              "GQL procedure input argument '%s' has an invalid type",
              argument.name);
        }
        input_project->projections.push_back(
            {std::move(bound), "gql_call_input_" + to_string(argument_index),
             expression->source});
      }
      if (definition->input_mode == GqlProcedureInputMode::ROW) {
        throw NotImplementedException(
            "Row-correlated GQL procedure execution is not implemented");
      }
      if (definition->input_mode == GqlProcedureInputMode::NONE) {
        if (!input_project->projections.empty()) {
          throw InternalException(
              "No-input GQL procedure has row input arguments");
        }
        ...
        input_project->projections.push_back(
            {std::move(marker), "gql_call_input_marker", call.source});
      ...
      for (const auto &yield : call.yield_items) {
        ...
        for (idx_t index = 0; index < definition->outputs.size(); index++) {
          if (StringUtil::CIEquals(definition->outputs[index].name,
                                   yield.field.value)) {
            output_index = index;
            break;
          }
        ...
        if (output_index == DConstants::INVALID_INDEX) {
          throw BinderException("GQL procedure '%s.%s' has no output '%s'",
                                definition->procedure_namespace,
                                definition->name, yield.field.value);
        }
        auto visible_name =
            yield.alias.IsEmpty() ? yield.field.value : yield.alias.value;
  ...
  for (auto is_bound : bound_patterns) {
    if (!is_bound) {
      throw InternalException(
          "GQL pattern is not owned by an ordered MATCH clause");
    }
  }
  for (auto is_bound : bound_predicate_indices) {
    if (!is_bound) {
      throw InternalException(
          "GQL predicate is not owned by an ordered query clause");
    }
  }
  ...
  if (!statement.procedure_calls.empty() && !saw_call_clause) {
    throw InternalException(
        "GQL procedure is not owned by an ordered CALL clause");
  }
  ...
  if (statement.has_mutation) {
    if (statement.insertion) {
      ...
      auto bind_properties = [&](const GqlInsertElement &element,
                                 const string &column_prefix,
                                 vector<GqlBoundInsertProperty> &properties) {
        case_insensitive_set_t names;
        for (idx_t property_index = 0;
             property_index < element.properties.size(); property_index++) {
          const auto &property = element.properties[property_index];
          if (!names.insert(property.name.value).second) {
            throw BinderException("Duplicate GQL INSERT property '%s'",
                                  property.name.value);
          }
          ...
          if (ContainsAggregate(*value)) {
            throw BinderException(
                "GQL aggregate functions are not allowed in INSERT");
          }
          if (!IsScalar(value->result_type.id)) {
            throw BinderException(
                "GQL INSERT property requires a scalar value");
          }
          auto value_column = column_prefix + std::to_string(property_index);
          project->projections.push_back(
              {std::move(value), value_column, property.source});
          properties.push_back(
              {property.name.value, std::move(value_column), property.source});
        }
      };

      for (idx_t vertex_index = 0;
           vertex_index < statement.insertion->vertices.size();
           vertex_index++) {
        ...
        auto existing = vertex.variable.IsEmpty()
                            ? DConstants::INVALID_INDEX
                            : FindBindingIndex(vertex.variable.value);
        if (existing != DConstants::INVALID_INDEX) {
          auto &binding = bindings[existing];
          ...
          if (statement.insertion->edges.empty() || !vertex.labels.empty() ||
              !vertex.properties.empty()) {
            throw BinderException(
                "GQL INSERT node variable '%s' is already bound",
                vertex.variable.value);
          }
          ...
          project->projections.push_back(
              {std::move(id), bound_vertex.existing_id_column, vertex.source});
        } else {
          ...
          if (!vertex.variable.IsEmpty()) {
            ...
            if (inserted != inserted_variables.end()) {
              if (!inserted->second.first) {
                throw BinderException(
                    "GQL INSERT variable '%s' is already defined as an edge",
                    vertex.variable.value);
              }
              if (!vertex.labels.empty() || !vertex.properties.empty()) {
                throw BinderException(
                    "GQL INSERT node variable '%s' is already defined",
                    vertex.variable.value);
              }
              bound_vertex.allocation_index = inserted->second.second;
              first_declaration = false;
            } else {
              bound_vertex.allocation_index =
                  bound_insertion->new_vertex_count++;
              inserted_variables.emplace(
                  vertex.variable.value,
          ...
          if (bound_vertex.create) {
            bind_properties(vertex,
                            "gql_insert_vertex_property_" +
                                std::to_string(vertex_index) + "_",
      ...
      for (idx_t edge_index = 0; edge_index < statement.insertion->edges.size();
           edge_index++) {
        ...
        if (edge.source_vertex >= bound_insertion->vertices.size() ||
          ...
          throw InternalException(
              "GQL INSERT edge endpoint is outside the vertex path");
        }
        if (!edge.variable.IsEmpty()) {
          if (FindBindingIndex(edge.variable.value) !=
                  DConstants::INVALID_INDEX ||
              inserted_variables.find(edge.variable.value) !=
                  inserted_variables.end()) {
            throw BinderException(
                "GQL INSERT edge variable '%s' is already bound",
                edge.variable.value);
          }
          inserted_variables.emplace(edge.variable.value,
                                     make_pair(false, edge_index));
        }
        ...
        bind_properties(edge,
                        "gql_insert_edge_property_" +
                            std::to_string(edge_index) + "_",
      ...
      project->projections.push_back(
          {std::move(row_marker), "gql_insert_row_marker", statement.source});
    } else {
      bool saw_null_delete_target = false;
      auto append_mutation = [&](const GqlMutation &mutation,
                                 idx_t binding_index, GqlType binding_type) {
        ...
        project->projections.push_back(
            {std::move(target_expression),
             "gql_target_id_" + std::to_string(mutation_index),
             mutation.source});
      ...
      auto append_delete_binding = [&](const GqlMutation &mutation,
        ...
        if (target.type.id == GqlTypeId::PATH) {
          ...
          if (path == path_bindings.end()) {
            throw InternalException(
                "GQL path mutation target has no bound elements");
          }
          for (const auto &element : path->second) {
            append_mutation(mutation, element.binding_index,
                            {element.type == GqlPatternElementType::VERTEX
                                 ? GqlTypeId::NODE
                                 : GqlTypeId::EDGE,
        ...
        if (target.type.id != GqlTypeId::NODE &&
            target.type.id != GqlTypeId::EDGE) {
          throw BinderException(
              "GQL DELETE target must resolve to a node, edge, or path");
        }
        append_mutation(mutation, target.index, target.type);
      };
      auto resolve_delete_selection =
          [&](auto &self,
              const GqlExpression &expression) -> const GqlExpression & {
        if (expression.type != GqlExpressionType::PROPERTY_REFERENCE) {
          return expression;
        }
        if (!expression.left) {
        ...
        if (receiver.type != GqlExpressionType::RECORD_CONSTRUCTOR ||
            receiver.field_names.size() != receiver.arguments.size()) {
          throw BinderException(
              "GQL DELETE field selection requires a record value");
        }
        for (idx_t field_index = 0; field_index < receiver.field_names.size();
             field_index++) {
          if (receiver.field_names[field_index].value ==
              expression.property.value) {
            return self(self, *receiver.arguments[field_index]);
          }
        }
        throw BinderException("GQL DELETE record field '%s' does not exist",
                              expression.property.value);
      };
      auto append_delete_target = [&](auto &self, const GqlMutation &mutation,
                                      const GqlExpression &expression) -> void {
        auto &target =
            resolve_delete_selection(resolve_delete_selection, expression);
        switch (target.type) {
        case GqlExpressionType::VARIABLE_REFERENCE:
      ...
      for (const auto &mutation : statement.mutations) {
        if (mutation.type == GqlMutationType::DELETE_ELEMENT) {
          if (!mutation.target) {
            throw InternalException(
                "GQL DELETE mutation has no target expression");
          }
          append_delete_target(append_delete_target, mutation,
                               *mutation.target);
          continue;
        }
        ...
        if (target.type.id == GqlTypeId::PATH) {
          throw BinderException(
              "GQL path mutation targets are only valid in DELETE");
        }
        if (target.type.id != GqlTypeId::NODE &&
            target.type.id != GqlTypeId::EDGE) {
        ...
        if (mutation.type == GqlMutationType::SET_PROPERTIES ||
          ...
          if (!mutation.value) {
            throw InternalException(
                "GQL property-map mutation has no value expression");
          }
          ...
          if (map.type != GqlExpressionType::RECORD_CONSTRUCTOR ||
            ...
            throw BinderException(
                "GQL property-map mutation requires a record value");
          }
          if (mutation.type == GqlMutationType::SET_PROPERTIES) {
            GqlMutation clear;
          ...
          for (idx_t field_index = 0; field_index < map.field_names.size();
               field_index++) {
          ...
          if (mutation.type == GqlMutationType::MERGE_PROPERTIES &&
              map.field_names.empty()) {
      ...
      if (bound_mutations.empty() && saw_null_delete_target) {
        ...
        project->projections.push_back(
            {std::move(null_target), "gql_target_id_0", null_mutation.source});
        bound_mutations.push_back(std::move(null_mutation));
      } else if (bound_mutations.empty()) {
        throw BinderException(
            "GQL DELETE collection target contains no elements");
      }
    }
    if (!statement.insertion) {
  ...
  for (const auto &order : statement.order_by) {
    ...
    for (idx_t index = 0; index < project->projections.size(); index++) {
      if (ExpressionsEqual(*bound_order.expression,
                           *project->projections[index].expression)) {
    ...
    if (bound_order.projection_index == DConstants::INVALID_INDEX) {
      throw BinderException(
          "GQL ORDER BY expressions must also appear in RETURN");
    }
