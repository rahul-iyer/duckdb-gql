...
GqlIdentifier GqlTransformer::TransformCallArgumentName(
    GQLParser::ProcedureArgumentContext &context) const {
  ...
  while (offset > 0 &&
         std::isspace(static_cast<unsigned char>(query[offset - 1]))) {
    offset--;
  }
  ...
  while (name_end > 0 &&
         std::isspace(static_cast<unsigned char>(query[name_end - 1]))) {
    name_end--;
  }
  auto name_start = name_end;
  while (name_start > 0 &&
         (std::isalnum(static_cast<unsigned char>(query[name_start - 1])) ||
          query[name_start - 1] == '_')) {
    name_start--;
  }
  if (name_start == name_end ||
      !(std::isalpha(static_cast<unsigned char>(query[name_start])) ||
        query[name_start] == '_')) {
    return result;
  }
  result.value =
      StringUtil::Lower(query.substr(name_start, name_end - name_start));
  result.source = SourceRange(name_start, name_end);
  return result;
}

static bool TransformPathBound(GQLParser::UnsignedIntegerContext *integer,
                               idx_t &result) {
...
static bool TransformNonNegativeInteger(
    GQLParser::NonNegativeIntegerSpecificationContext *specification,
    idx_t &result) {
...
static void CollectContexts(antlr4::tree::ParseTree *tree,
...
shared_ptr<GqlStatement>
GqlTransformer::Transform(GQLParser::GqlProgramContext &root) {
  statement.reset();
  vector<GQLParser::NamedProcedureCallContext *> procedure_calls;
  vector<GQLParser::SimpleMatchStatementContext *> match_statements;
  vector<GQLParser::PrimitiveDataModifyingStatementContext *>
...
std::any GqlTransformer::visitCreateGraphStatement(
    GQLParser::CreateGraphStatementContext *context) {
  if (context->PROPERTY()) {
  ...
  if (!context->openGraphType() || !context->openGraphType()->ANY() ||
      context->ofGraphType() || context->graphSource()) {
  ...
  statement = make_shared_ptr<GqlCreateGraphStatement>(
      SourceRange(*context), TransformIdentifier(*graph_name),
      context->IF() != nullptr);
  return {};
}

std::any GqlTransformer::visitDropGraphStatement(
    GQLParser::DropGraphStatementContext *context) {
  if (context->PROPERTY()) {
    Unsupported(*context, "PROPERTY GRAPH spelling");
    return {};
  }
  ...
  statement = make_shared_ptr<GqlDropGraphStatement>(
      SourceRange(*context), TransformIdentifier(*graph_name),
      context->IF() != nullptr);
  return {};
}

std::any GqlTransformer::visitSessionSetGraphClause(
    GQLParser::SessionSetGraphClauseContext *context) {
  if (context->PROPERTY()) {
    Unsupported(*context, "PROPERTY GRAPH spelling");
    return {};
  }
  ...
  statement = make_shared_ptr<GqlSessionSetGraphStatement>(
      SourceRange(*context), TransformIdentifier(*expression));
  return {};
}

std::any GqlTransformer::visitInsertStatement(
    GQLParser::InsertStatementContext *context) {
  statement = TransformInsert(*context, false);
  return {};
}

shared_ptr<GqlInsertStatement>
GqlTransformer::TransformInsert(GQLParser::InsertStatementContext &context,
  ...
  for (idx_t index = 0; index < edge_patterns.size(); index++) {
    ...
    } else if (auto pointing_left =
                   edge_patterns[index]->insertEdgePointingLeft()) {
...
bool GqlTransformer::TransformInsertElement(
    GQLParser::InsertElementPatternFillerContext *filler,
  ...
  for (auto property : property_specification->propertyKeyValuePairList()
    ...
    if (allow_expressions) {
      if (!TransformExpression(*property->valueExpression(),
                               assignment.expression)) {
        Unsupported(*property, "INSERT property expression");
        return false;
      }
    } else if (!TransformLiteral(*property->valueExpression(),
                                 assignment.value)) {
...
bool GqlTransformer::TransformCall(GQLParser::GqlProgramContext &root) {
  ...
  if (procedure_calls.size() != 1 || return_statements.size() != 1 ||
  ...
  auto procedure_name_text = separator == string::npos
                                 ? string()
                                 : reference_text.substr(separator + 1);
  if (separator == string::npos || separator != reference_text.rfind('.') ||
      namespace_text.empty() || procedure_name_text.empty() ||
      namespace_text.front() == '"' || namespace_text.front() == '`' ||
      procedure_name_text.front() == '"' ||
      procedure_name_text.front() == '`' ||
      !IsRegularIdentifier(namespace_text) ||
      !IsRegularIdentifier(procedure_name_text) ||
  ...
  if (auto arguments = procedure->procedureArgumentList()) {
    for (auto argument : arguments->procedureArgument()) {
      GqlLiteral literal;
      if (!argument->valueExpression() ||
          !TransformLiteral(*argument->valueExpression(), literal)) {
        return fail();
      }
  ...
  for (auto item : yield->yieldItemList()->yieldItem()) {
    ...
    if (auto alias = item->yieldItemAlias()) {
      if (!alias->bindingVariable() ||
          !IsRegularIdentifier(alias->bindingVariable()->getText())) {
        return fail();
      }
      yield_item.alias = TransformIdentifier(*alias->bindingVariable());
    }
    auto output_name = yield_item.alias.IsEmpty() ? yield_item.field.value
                                                  : yield_item.alias.value;
  ...
  if (!body || body->ASTERISK() || !body->returnItemList() ||
      body->groupByClause()) {
    return fail();
  }
  ...
  for (auto item : body->returnItemList()->returnItem()) {
    ...
    if (!TransformProjection(item, projection) ||
        ...
        yielded_names.find(projection.expression->variable.value) ==
            yielded_names.end()) {
      return fail();
    }
    auto output_name = projection.alias.IsEmpty()
                           ? projection.expression->variable.value
  ...
  if (!order_statements.empty()) {
    ...
    if (auto order_clause = order_page->orderByClause()) {
      for (auto specification :
           order_clause->sortSpecificationList()->sortSpecification()) {
        GqlOrderBy order;
        order.source = SourceRange(*specification);
        auto expression = specification->sortKey()
    ...
    if (auto offset = order_page->offsetClause()) {
      if (!TransformNonNegativeInteger(
              offset->nonNegativeIntegerSpecification(), call->offset)) {
        return fail();
      }
      call->has_offset = true;
    }
    if (auto limit = order_page->limitClause()) {
      if (!TransformNonNegativeInteger(limit->nonNegativeIntegerSpecification(),
                                       call->limit)) {
        return fail();
      }
      call->has_limit = true;
...
bool GqlTransformer::TransformMatch(GQLParser::GqlProgramContext &root) {
  ...
  auto base_primitives = match_statements.size() + let_statements.size() +
                         filter_statements.size();
  auto expected_primitives = base_primitives + procedure_calls.size();
  if (modifying_statements.size() > 1 ||
      procedure_calls.size() > 1 || (has_mutation && !procedure_calls.empty()) ||
      (has_mutation ? !return_statements.empty() || !order_statements.empty()
                    : return_statements.size() != 1) ||
      order_statements.size() > 1 ||
      (primitive_statements.size() != base_primitives &&
       primitive_statements.size() !=
           base_primitives + order_statements.size() &&
       primitive_statements.size() != expected_primitives &&
       primitive_statements.size() !=
           expected_primitives + order_statements.size())) {
    return fail();
  }

  auto match = make_shared_ptr<GqlMatchStatement>(SourceRange(root));
  auto append_match = [&](GQLParser::SimpleMatchStatementContext &simple_match,
    ...
    if (binding_table->graphPatternYieldClause() ||
        graph_pattern->matchMode() || graph_pattern->keepClause()) {
      return false;
    }
    ...
    for (auto path : paths) {
      ...
      auto path_expression = dynamic_cast<GQLParser::PpePathTermContext *>(
          path->pathPatternExpression());
      if (!path_expression) {
        return false;
      }
      ...
      if (auto declaration = path->pathVariableDeclaration()) {
        ast_pattern.variable =
            TransformIdentifier(*declaration->pathVariable());
      }
      for (idx_t index = 0; index < factors.size(); index++) {
        ...
        if (auto factor = dynamic_cast<GQLParser::PfPathPrimaryContext *>(
                factors[index])) {
          path_primary = factor->pathPrimary();
        } else if (auto factor = dynamic_cast<
        ...
        auto primary =
            dynamic_cast<GQLParser::PpElementPatternContext *>(path_primary);
        if (!primary) {
          return false;
        }
        ...
        if (index % 2 == 0) {
          if (!element->nodePattern() ||
              !TransformMatchElement(
                  element->nodePattern()->elementPatternFiller(),
          ...
          if (auto full = edge->fullEdgePattern()) {
            ...
            if (abbreviated->RIGHT_ARROW()) {
              ...
              ast_element.edge_direction = GqlEdgeDirection::LEFT;
            } else if (abbreviated->MINUS_SIGN() ||
                       abbreviated->LEFT_MINUS_RIGHT()) {
              ast_element.edge_direction = GqlEdgeDirection::ANY;
            } else {
              return false;
            }
          ...
          if (!TransformMatchElement(filler, GqlPatternElementType::EDGE,
                                     ast_element)) {
            return false;
          }
        }
        if (quantifier) {
          ...
          if (quantifier->ASTERISK()) {
            ...
            if (!TransformPathBound(fixed->unsignedInteger(),
                                    minimum_repetitions) ||
                minimum_repetitions == 0) {
              Unsupported(
                  *quantifier,
                  "fixed path quantifier outside the supported range 1..64");
              return false;
            }
            ...
            if (general->lowerBound() &&
                !TransformPathBound(general->lowerBound()->unsignedInteger(),
                                    minimum_repetitions)) {
              Unsupported(
                  *quantifier,
                  "path quantifier bounds outside the supported range 1..64");
              return false;
            }
            if (!general->upperBound()) {
              maximum_repetitions = minimum_repetitions;
              unbounded = true;
            } else if (!TransformPathBound(
                           general->upperBound()->unsignedInteger(),
                           maximum_repetitions) ||
  ...
  for (auto match_statement : match_statements) {
    ...
    if (match_statement->simpleMatchStatement()) {
      ...
      saw_mandatory = true;
    } else if (auto optional_match =
                   match_statement->optionalMatchStatement()) {
    ...
    if (!append_match(*simple_match, optional, optional ? optional_stage : 0,
                      clause)) {
      return fail();
    }
  ...
  for (auto let : let_statements) {
    ...
    for (auto definition :
         let->letVariableDefinitionList()->letVariableDefinition()) {
      if (definition->valueVariableDefinition() ||
          !definition->bindingVariable() || !definition->valueExpression() ||
          !IsRegularIdentifier(definition->bindingVariable()->getText())) {
        return fail();
      }
      ...
      if (!TransformExpression(*definition->valueExpression(),
                               binding.expression)) {
        return fail();
      }
  ...
  for (auto procedure : procedure_calls) {
    ...
    if (auto arguments = procedure->procedureArgumentList()) {
      for (auto argument : arguments->procedureArgument()) {
        ...
        if (!argument->valueExpression() ||
            !TransformExpression(*argument->valueExpression(), expression)) {
          return fail();
        }
  ...
  std::stable_sort(match->clauses.begin(), match->clauses.end(),
                   [](const GqlQueryClause &left, const GqlQueryClause &right) {
                     return left.source.start_offset <
                            right.source.start_offset;
                   });
  ...
  for (auto &clause : match->clauses) {
    ...
    if (clause.type == GqlQueryClauseType::CALL) {
      if (!saw_match_clause || saw_call_clause ||
          clause.procedure_call_index >= match->procedure_calls.size()) {
        return fail();
      }
    ...
    if (clause.type != GqlQueryClauseType::FILTER || !saw_match_clause ||
        saw_call_clause || clause.predicate_index >= match->predicates.size()) {
      return fail();
    }
  ...
  if (!order_statements.empty()) {
        ...
        if (order.expression->type == GqlExpressionType::VARIABLE_REFERENCE) {
          for (const auto &projection : match->projections) {
            if (!projection.alias.IsEmpty() &&
                projection.alias.value == order.expression->variable.value) {
    ...
    if (auto offset = order_page->offsetClause()) {
      if (!TransformNonNegativeInteger(
              offset->nonNegativeIntegerSpecification(), match->offset)) {
...
bool GqlTransformer::TransformMutation(
    GQLParser::PrimitiveDataModifyingStatementContext &context,
    ...
    for (auto item : items) {
      ...
      if (auto property = item->setPropertyItem()) {
        if (!IsRegularIdentifier(
                property->bindingVariableReference()->getText()) ||
        ...
        mutation.variable =
            TransformIdentifier(*property->bindingVariableReference());
        ...
        auto equals_offset =
            all->EQUALS_OPERATOR()->getSymbol()->getStartIndex();
        auto merge = equals_offset > 0 && equals_offset <= query.size() &&
                     query[equals_offset - 1] == '+';
        ...
        auto skip_trivia = [&]() {
          while (rhs_offset < query.size()) {
            ...
            if (rhs_offset + 1 < query.size() && query[rhs_offset] == '-' &&
                query[rhs_offset + 1] == '-') {
              rhs_offset += 2;
        ...
        if (query[rhs_offset] != '{') {
          ...
          if (delimited) {
            ...
            while (rhs_offset < query.size()) {
              ...
              if (rhs_offset + 1 < query.size() &&
                  query[rhs_offset + 1] == quote) {
                rhs_offset += 2;
                continue;
              }
              rhs_offset++;
            ...
            identifier = UnquoteString(
                query.substr(identifier_start, rhs_offset - identifier_start));
          } else {
            ...
            while (
                ...
                (std::isalnum(static_cast<unsigned char>(query[rhs_offset])) ||
                 query[rhs_offset] == '_')) {
              rhs_offset++;
            }
            identifier =
          ...
          expression->variable.source =
              SourceRange(identifier_start, rhs_offset);
          expression->source = expression->variable.source;
          mutation.type = merge ? GqlMutationType::MERGE_PROPERTIES
                                : GqlMutationType::SET_PROPERTIES;
          mutation.variable = std::move(variable);
          mutation.value = std::move(expression);
          mutation.source =
              SourceRange(mutation.source.start_offset, rhs_offset);
          match.mutations.push_back(std::move(mutation));
          continue;
        }
        if (!merge) {
        ...
        if (!IsRegularIdentifier(
                label->bindingVariableReference()->getText()) ||
...
bool GqlTransformer::TransformMatchElement(
    GQLParser::ElementPatternFillerContext *filler, GqlPatternElementType type,
  ...
  if (auto is_label = filler->isLabelExpression()) {
    if (!TransformLabelExpression(*is_label->labelExpression(),
                                  result.labels)) {
      return false;
    }
  }
...
bool GqlTransformer::TransformLabelExpression(
    GQLParser::LabelExpressionContext &context, vector<GqlIdentifier> &labels) {
  if (auto name =
          dynamic_cast<GQLParser::LabelExpressionNameContext *>(&context)) {
...
bool GqlTransformer::TransformProjection(GQLParser::ReturnItemContext *item,
                                         GqlProjection &result) {
...
bool GqlTransformer::TransformSearchCondition(
    GQLParser::SearchConditionContext &context,
    shared_ptr<GqlExpression> &result) {
  if (!context.booleanValueExpression()) {
    return false;
  }
  return TransformExpression(
      *context.booleanValueExpression()->valueExpression(), result);
}

bool GqlTransformer::TransformExpression(
    GQLParser::ValueExpressionContext &context,
    shared_ptr<GqlExpression> &result) {
  auto expression = make_shared_ptr<GqlExpression>();
  expression->source = SourceRange(context);
  if (auto primary =
          dynamic_cast<GQLParser::PrimaryExprAltContext *>(&context)) {
    return TransformExpressionPrimary(*primary->valueExpressionPrimary(),
                                      result);
  }
  if (auto function =
          dynamic_cast<GQLParser::ValueFunctionExprAltContext *>(&context)) {
    return TransformValueFunction(*function->valueFunction(), result);
  }
  if (auto signed_expression =
          dynamic_cast<GQLParser::SignedExprAltContext *>(&context)) {
    expression->type = GqlExpressionType::UNARY;
    expression->unary_operator = signed_expression->MINUS_SIGN()
                                     ? GqlUnaryOperator::MINUS
                                     : GqlUnaryOperator::PLUS;
    if (!TransformExpression(*signed_expression->valueExpression(),
                             expression->left)) {
      return false;
    }
  ...
  if (auto not_expression =
          dynamic_cast<GQLParser::NotExprAltContext *>(&context)) {
    expression->type = GqlExpressionType::UNARY;
    expression->unary_operator = GqlUnaryOperator::NOT;
    if (!TransformExpression(*not_expression->valueExpression(),
                             expression->left)) {
      return false;
    }
    result = std::move(expression);
  ...
  auto transform_binary = [&](GQLParser::ValueExpressionContext &left,
                              GQLParser::ValueExpressionContext &right,
                              GqlBinaryOperator operation) {
    expression->type = GqlExpressionType::BINARY;
    expression->binary_operator = operation;
    if (!TransformExpression(left, expression->left) ||
        !TransformExpression(right, expression->right)) {
      return false;
    }
    result = std::move(expression);
    return true;
  ...
  if (auto mult = dynamic_cast<GQLParser::MultDivExprAltContext *>(&context)) {
    ...
    return transform_binary(*children[0], *children[1],
                            mult->ASTERISK() ? GqlBinaryOperator::MULTIPLY
                                             : GqlBinaryOperator::DIVIDE);
  }
  if (auto add =
          dynamic_cast<GQLParser::AddSubtractExprAltContext *>(&context)) {
    auto children = add->valueExpression();
    return transform_binary(*children[0], *children[1],
                            add->PLUS_SIGN() ? GqlBinaryOperator::ADD
                                             : GqlBinaryOperator::SUBTRACT);
  }
  if (auto concatenate =
          dynamic_cast<GQLParser::ConcatenationExprAltContext *>(&context)) {
    auto children = concatenate->valueExpression();
    return transform_binary(*children[0], *children[1],
                            GqlBinaryOperator::CONCATENATE);
  }
  if (auto comparison =
          dynamic_cast<GQLParser::ComparisonExprAltContext *>(&context)) {
  ...
  if (auto conjunction =
          dynamic_cast<GQLParser::ConjunctiveExprAltContext *>(&context)) {
    auto children = conjunction->valueExpression();
    return transform_binary(*children[0], *children[1], GqlBinaryOperator::AND);
  }
  if (auto disjunction =
          dynamic_cast<GQLParser::DisjunctiveExprAltContext *>(&context)) {
    auto children = disjunction->valueExpression();
    return transform_binary(*children[0], *children[1],
                            disjunction->OR() ? GqlBinaryOperator::OR
                                              : GqlBinaryOperator::XOR);
  }
  if (auto predicate_expression =
          dynamic_cast<GQLParser::PredicateExprAltContext *>(&context)) {
    ...
    if (auto null_predicate = predicate->nullPredicate()) {
      ...
      expression->negated =
          null_predicate->nullPredicatePart2()->NOT() != nullptr;
      if (!TransformExpressionPrimary(*null_predicate->valueExpressionPrimary(),
                                      expression->left)) {
        return false;
      }
      result = std::move(expression);
    ...
    if (auto labeled = predicate->labeledPredicate()) {
      ...
      if (!TransformLabelExpression(*part->labelExpression(), labels) ||
          labels.empty()) {
        return false;
      }
...
bool GqlTransformer::TransformExpressionPrimary(
    GQLParser::ValueExpressionPrimaryContext &context,
    shared_ptr<GqlExpression> &result) {
  ...
  if (auto specification = context.unsignedValueSpecification()) {
    ...
    if (auto general = literal->generalLiteral()) {
      if (auto list = general->listLiteral()) {
        return TransformListConstructor(
            *list->listValueConstructorByEnumeration(), result);
      }
  ...
  if (context.propertyName() && context.valueExpressionPrimary()) {
    if (!IsRegularIdentifier(context.propertyName()->getText()) ||
        !TransformExpressionPrimary(*context.valueExpressionPrimary(),
                                    expression->left)) {
      return false;
    }
...
bool GqlTransformer::TransformListConstructor(
    GQLParser::ListValueConstructorByEnumerationContext &context,
    shared_ptr<GqlExpression> &result) {
  auto expression = make_shared_ptr<GqlExpression>();
...
bool GqlTransformer::TransformRecordConstructor(
    GQLParser::RecordConstructorContext &context,
    shared_ptr<GqlExpression> &result) {
  auto expression = make_shared_ptr<GqlExpression>();
  ...
  if (auto field_list = fields->fieldList()) {
    for (auto field : field_list->field()) {
      if (!field->fieldName() ||
          !IsRegularIdentifier(field->fieldName()->getText())) {
        return false;
      }
...
bool GqlTransformer::TransformAggregate(
    GQLParser::AggregateFunctionContext &context,
    shared_ptr<GqlExpression> &result) {
  auto expression = make_shared_ptr<GqlExpression>();
  ...
  if (!general || !general->generalSetFunctionType() ||
      !general->valueExpression()) {
    return false;
  }
  expression->function_name =
      StringUtil::Lower(general->generalSetFunctionType()->getText());
  expression->distinct =
      general->setQuantifier() && general->setQuantifier()->DISTINCT();
...
bool GqlTransformer::TransformNumericExpression(
    GQLParser::NumericValueExpressionContext &context,
    shared_ptr<GqlExpression> &result) {
  if (context.valueExpressionPrimary()) {
    return TransformExpressionPrimary(*context.valueExpressionPrimary(),
                                      result);
  }
  ...
  if (children.size() == 1 && (context.PLUS_SIGN() || context.MINUS_SIGN())) {
    ...
    expression->unary_operator =
        context.MINUS_SIGN() ? GqlUnaryOperator::MINUS : GqlUnaryOperator::PLUS;
    expression->source = SourceRange(context);
...
bool GqlTransformer::TransformNumericFunction(
    GQLParser::NumericValueFunctionContext &context,
    shared_ptr<GqlExpression> &result) {
  auto make_function = [&](const string &name,
                           GQLParser::NumericValueExpressionContext &argument) {
    auto expression = make_shared_ptr<GqlExpression>();
    expression->type = GqlExpressionType::FUNCTION;
  ...
  if (auto modulus = context.modulusExpression()) {
    ...
    if (!TransformNumericExpression(*modulus->numericValueExpressionDividend()
                                         ->numericValueExpression(),
                                    left) ||
        !TransformNumericExpression(
            *modulus->numericValueExpressionDivisor()->numericValueExpression(),
            right)) {
      return false;
    }
  ...
  if (auto length = context.lengthExpression()) {
    ...
    if (!TransformExpression(
            *character->characterStringValueExpression()->valueExpression(),
            child)) {
      return false;
    }
    expression->arguments.push_back(std::move(child));
...
bool GqlTransformer::TransformValueFunction(
    GQLParser::ValueFunctionContext &context,
    shared_ptr<GqlExpression> &result) {
  if (context.numericValueFunction()) {
    return TransformNumericFunction(*context.numericValueFunction(), result);
  }
  ...
  if (auto fold = string_function->foldCharacterString()) {
    ...
    if (!TransformExpression(*substring->valueExpression(), value) ||
        !TransformNumericExpression(
            *substring->stringLength()->numericValueExpression(), length)) {
      return false;
    }
    ...
    expression->function_name = trim->LTRIM()   ? "ltrim"
                                : trim->RTRIM() ? "rtrim"
                                                : "trim";
    ...
    expression->function_name =
        operands->trimSpecification() &&
                operands->trimSpecification()->LEADING()
            ? "ltrim"
        : operands->trimSpecification() &&
                operands->trimSpecification()->TRAILING()
            ? "rtrim"
            : "trim";
    shared_ptr<GqlExpression> source;
    if (!TransformExpression(
            *operands->trimCharacterOrByteStringSource()->valueExpression(),
            source)) {
      return false;
    }
    ...
    if (operands->trimCharacterOrByteString()) {
      ...
      if (!TransformExpression(
              *operands->trimCharacterOrByteString()->valueExpression(),
              characters)) {
        return false;
      }
...
bool GqlTransformer::TransformUnsignedLiteral(
    GQLParser::UnsignedLiteralContext &context, GqlLiteral &result) {
  result.source = SourceRange(context);
  if (auto numeric = context.unsignedNumericLiteral()) {
    if (auto exact = numeric->exactNumericLiteral()) {
      if (exact->unsignedInteger() &&
          exact->unsignedInteger()->UNSIGNED_DECIMAL_INTEGER()) {
...
bool GqlTransformer::TransformLiteral(
    GQLParser::ValueExpressionContext &context, GqlLiteral &result) {
  result.source = SourceRange(context);
  if (auto signed_expression =
          dynamic_cast<GQLParser::SignedExprAltContext *>(&context)) {
    GqlLiteral unsigned_literal;
    if (!TransformLiteral(*signed_expression->valueExpression(),
                          unsigned_literal) ||
        (unsigned_literal.type != GqlLiteralType::INTEGER &&
         unsigned_literal.type != GqlLiteralType::DECIMAL &&
  ...
  auto specification = primary_expression->valueExpressionPrimary()
...
bool GqlTransformer::TransformChainedLabels(
    antlr4::ParserRuleContext &first_label,
  ...
  while (offset < query.size() && query[offset] == ':') {
    ...
    label.value =
        StringUtil::Lower(label.delimited ? UnquoteString(text) : text);
...
void GqlTransformer::Unsupported(antlr4::ParserRuleContext &context,
                                 const string &feature) {
  statement =
      make_shared_ptr<GqlUnsupportedStatement>(SourceRange(context), feature);
}
...
string GqlTransformer::UnquoteString(const string &text) {
  ...
  for (idx_t index = 1; index + 1 < text.size(); index++) {
    if (text[index] == quote && index + 2 < text.size() &&
        text[index + 1] == quote) {
      result += quote;
      index++;
    } else {
...
GqlIdentifier
GqlTransformer::TransformIdentifier(antlr4::ParserRuleContext &context) {
  GqlIdentifier result;
  auto text = context.getText();
  result.delimited = !text.empty() && (text[0] == '`' || text[0] == '"');
  result.value =
      StringUtil::Lower(result.delimited ? UnquoteString(text) : text);
  result.source = SourceRange(context);
  return result;
}
