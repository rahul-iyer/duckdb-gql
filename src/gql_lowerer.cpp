...
ParserExtensionPlanResult GqlLower(const GqlLogicalPlan &plan) {
  ...
  if (!current || current->type != GqlLogicalOperatorType::MATCH ||
      current->child) {
  ...
  if (!recursive_edge && plan.execution_mode == GqlExecutionMode::CSR) {
    throw NotImplementedException("CSR execution hint currently requires one "
                                  "anonymous unbounded directed path factor");
  }
  ...
  vector<Value> optional_filter_programs;
  vector<Value> optional_filter_stages;
  if (match.optional_predicates.size() !=
      match.optional_predicate_stages.size()) {
    throw InternalException("GQL optional predicate stage metadata is inconsistent");
  }
  for (idx_t index = 0; index < match.optional_predicates.size(); index++) {
    optional_filter_programs.push_back(
        GqlSerializeExpression(*match.optional_predicates[index]));
    optional_filter_stages.emplace_back(
        Value::UBIGINT(match.optional_predicate_stages[index]));
  }
  ...
  if (recursive_edge) {
    ...
    result.parameters.emplace_back(plan.execution_mode ==
                                   GqlExecutionMode::CSR);
  ...
  vector<Value> optionals;
  vector<Value> optional_stages;
  for (const auto &pattern : match.patterns) {
    pattern_sizes.emplace_back(Value::UBIGINT(pattern.elements.size()));
    optionals.emplace_back(pattern.optional);
    optional_stages.emplace_back(Value::UBIGINT(pattern.optional_stage));
    for (const auto &element : pattern.elements) {
  ...
  result.parameters.emplace_back(
      Value::LIST(LogicalType::BOOLEAN, std::move(optionals)));
  result.parameters.emplace_back(
      Value::LIST(LogicalType::UBIGINT, std::move(optional_stages)));
  result.parameters.emplace_back(
  ...
  result.parameters.emplace_back(
      Value::LIST(GqlExpressionProgramType(), std::move(filter_programs)));
  result.parameters.emplace_back(Value::LIST(GqlExpressionProgramType(),
                                             std::move(optional_filter_programs)));
  result.parameters.emplace_back(
      Value::LIST(LogicalType::UBIGINT, std::move(optional_filter_stages)));
  append_result_modifiers(result.parameters);
  return result;
}
