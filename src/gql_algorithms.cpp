...
static const vector<GqlProcedureDefinition> &AlgorithmProcedures() {
  static const vector<GqlProcedureDefinition> procedures = {
      {"algo",
       ...
       {{"graph",
         {GqlTypeId::STRING, false},
         GqlProcedureArgumentMode::CONFIGURATION},
        {"frontier",
         {GqlTypeId::ELEMENT_ID, false},
         GqlProcedureArgumentMode::INPUT},
        {"vertex_label",
         {GqlTypeId::STRING, false},
         GqlProcedureArgumentMode::CONFIGURATION,
         true}},
...
GqlFindProcedure(const string &procedure_namespace, const string &name) {
  for (const auto &procedure : AlgorithmProcedures()) {
    if (StringUtil::CIEquals(procedure.procedure_namespace,
                             procedure_namespace) &&
...
static const Value *NamedParameter(const TableFunctionBindInput &input,
                                   const string &name) {
  auto entry = input.named_parameters.find(name);
  return entry == input.named_parameters.end() ? nullptr : &entry->second;
}

static string ReadLabelParameter(const TableFunctionBindInput &input,
                                 const string &name) {
  auto value = NamedParameter(input, name);
  return value && !value->IsNull()
             ? StringUtil::Lower(value->GetValue<string>())
             : string();
}

static string ReadVertexLabel(const TableFunctionBindInput &input,
                              idx_t positional_index) {
  auto named = ReadLabelParameter(input, "vertex_label");
  string positional;
  if (input.inputs.size() > positional_index &&
      !input.inputs[positional_index].IsNull()) {
    positional =
        StringUtil::Lower(input.inputs[positional_index].GetValue<string>());
  }
  if (!named.empty() && !positional.empty()) {
    throw BinderException(
        "GQL algorithm vertex_label cannot be both positional and named");
  }
...
static CsrDirection ReadDirection(const TableFunctionBindInput &input) {
  ...
  auto direction = value && !value->IsNull()
                       ? StringUtil::Lower(value->GetValue<string>())
                       : "out";
  if (direction == "out") {
  ...
  throw BinderException(
      "GQL CSR traversal direction must be 'out', 'in', or 'both'");
}
...
static unique_ptr<FunctionData> TraversalBind(ClientContext &,
  ...
  if ((input.inputs.size() != 2 && input.inputs.size() != 3) ||
    ...
    throw BinderException(
        "GQL CSR traversal requires a graph name and start vertex ID");
  }
...
static unique_ptr<FunctionData> SsspBind(ClientContext &context,
                                         TableFunctionBindInput &input,
                                         vector<LogicalType> &return_types,
                                         vector<string> &names) {
  auto result = TraversalBind(context, input, return_types, names);
  names = {"vertex_id", "distance", "parent_vertex_id", "edge_id",
           "settled_order"};
  return result;
}

static uint32_t ResolveLabel(const GqlCsrSnapshot &snapshot,
                             const string &label_name, bool &filter_label) {
  filter_label = !label_name.empty();
  if (!filter_label) {
    return 0;
  }
  auto label = snapshot.label_ids.find(label_name);
  return label == snapshot.label_ids.end()
             ? std::numeric_limits<uint32_t>::max()
             : label->second;
}

static vector<uint8_t> BuildVertexMask(const GqlCsrSnapshot &snapshot,
...
template <class CALLBACK>
static void VisitNeighbors(const GqlCsrSnapshot &snapshot, idx_t vertex,
                           CsrDirection direction, bool filter_label,
                           uint32_t required_label, CALLBACK &&callback) {
  if (direction != CsrDirection::IN &&
      !VisitRange(snapshot.outgoing_offsets, snapshot.outgoing_neighbors,
                  snapshot.outgoing_edge_ids, snapshot.outgoing_label_ids,
                  vertex, filter_label, required_label, callback)) {
    return;
  }
  if (direction != CsrDirection::OUT) {
    VisitRange(snapshot.incoming_offsets, snapshot.incoming_neighbors,
               snapshot.incoming_edge_ids, snapshot.incoming_label_ids, vertex,
               filter_label, required_label, callback);
  }
}

static idx_t RequireVertex(const GqlCsrSnapshot &snapshot, uint64_t vertex_id,
                           const string &role) {
  idx_t ordinal;
...
static idx_t RequireProjectedVertex(const GqlCsrSnapshot &snapshot,
  ...
  if (!InVertexProjection(vertex_mask, vertex)) {
    throw InvalidInputException(
        "GQL CSR %s vertex %llu is outside the vertex-label projection", role,
        static_cast<unsigned long long>(vertex_id));
  }
  return vertex;
}
...
struct TraversalOutputWriter {
  ...
  void Write(idx_t row_index, const GqlCsrSnapshot &snapshot,
             const TraversalRow &row, idx_t visit_order) {
...
static unique_ptr<GlobalTableFunctionState> BfsInit(ClientContext &,
                                                    TableFunctionInitInput &) {
  return make_uniq<BfsState>();
}

static void InitializeBfs(ClientContext &context, const TraversalBindData &data,
                          BfsState &state) {
  state.snapshot = GqlGetCsrSnapshot(context, data.graph_name);
  idx_t projected_count;
  state.vertex_mask =
      BuildVertexMask(*state.snapshot, data.vertex_label, projected_count);
  auto start = RequireProjectedVertex(*state.snapshot, state.vertex_mask,
                                      data.start_vertex_id, "start");
  state.has_target = data.has_target;
  if (data.has_target) {
    state.target = RequireProjectedVertex(*state.snapshot, state.vertex_mask,
                                          data.target_vertex_id, "target");
  }
  state.required_label =
      ResolveLabel(*state.snapshot, data.edge_label, state.filter_label);
...
static void BfsFunction(ClientContext &context, TableFunctionInput &input,
                        DataChunk &output) {
  ...
  idx_t count = 0;
  while (count < STANDARD_VECTOR_SIZE && state.head < state.queue.size() &&
         !state.finished) {
    ...
    VisitNeighbors(*state.snapshot, row.vertex, data.direction,
                   state.filter_label, state.required_label,
                   [&](idx_t neighbor, uint64_t edge_id) {
                     ...
                     state.queue.push_back(
                         {neighbor, row.depth + 1, row.vertex, edge_id, true});
                     return true;
                   });
  }
...
static unique_ptr<GlobalTableFunctionState> DfsInit(ClientContext &,
                                                    TableFunctionInitInput &) {
  return make_uniq<DfsState>();
}

static DfsFrame MakeDfsFrame(const GqlCsrSnapshot &snapshot, idx_t vertex,
                             idx_t depth, CsrDirection direction) {
  if (direction == CsrDirection::IN) {
    return {vertex, depth, 1, snapshot.incoming_offsets[vertex],
            snapshot.incoming_offsets[vertex + 1]};
  }
  return {vertex, depth, 0, snapshot.outgoing_offsets[vertex],
          snapshot.outgoing_offsets[vertex + 1]};
}

static bool NextDfsNeighbor(const GqlCsrSnapshot &snapshot,
                            CsrDirection direction, bool filter_label,
                            uint32_t required_label,
                            const vector<uint8_t> &vertex_mask, DfsFrame &frame,
                            idx_t &neighbor, uint64_t &edge_id) {
  while (true) {
    const auto &neighbors = frame.phase == 0 ? snapshot.outgoing_neighbors
                                             : snapshot.incoming_neighbors;
    const auto &edge_ids = frame.phase == 0 ? snapshot.outgoing_edge_ids
                                            : snapshot.incoming_edge_ids;
    const auto &label_ids = frame.phase == 0 ? snapshot.outgoing_label_ids
                                             : snapshot.incoming_label_ids;
...
static void InitializeDfs(ClientContext &context, const TraversalBindData &data,
                          DfsState &state) {
  state.snapshot = GqlGetCsrSnapshot(context, data.graph_name);
  idx_t projected_count;
  state.vertex_mask =
  ...
  state.stack.push_back(
      MakeDfsFrame(*state.snapshot, start, 0, data.direction));
  state.initialized = true;
}

static void DfsFunction(ClientContext &context, TableFunctionInput &input,
                        DataChunk &output) {
  auto &data = input.bind_data->Cast<TraversalBindData>();
  ...
  if (state.emit_start && count < STANDARD_VECTOR_SIZE) {
    writer.Write(count++, *state.snapshot, state.start_row,
                 state.visit_order++);
  ...
  while (count < STANDARD_VECTOR_SIZE && !state.finished &&
    ...
    idx_t neighbor;
    uint64_t edge_id;
    if (!NextDfsNeighbor(*state.snapshot, data.direction, state.filter_label,
                         state.required_label, state.vertex_mask, frame,
                         neighbor, edge_id)) {
      state.stack.pop_back();
      continue;
    }
    ...
    state.stack.push_back(
        MakeDfsFrame(*state.snapshot, neighbor, row.depth, data.direction));
    writer.Write(count++, *state.snapshot, row, state.visit_order++);
...
static unique_ptr<FunctionData> PageRankBind(ClientContext &,
                                             TableFunctionBindInput &input,
                                             vector<LogicalType> &return_types,
                                             vector<string> &names) {
  ...
  if (!std::isfinite(result->damping) || result->damping <= 0.0 ||
      result->damping >= 1.0) {
  ...
  return_types = {LogicalType::UBIGINT, LogicalType::DOUBLE,
                  LogicalType::UBIGINT, LogicalType::BOOLEAN};
  return std::move(result);
}
...
PageRankInit(ClientContext &, TableFunctionInitInput &) {
  return make_uniq<PageRankState>();
}

static void ComputePageRank(ClientContext &context,
                            const PageRankBindData &data,
                            PageRankState &state) {
  state.snapshot = GqlGetCsrSnapshot(context, data.graph_name);
  ...
  bool filter_label;
  auto required_label =
      ResolveLabel(*state.snapshot, data.edge_label, filter_label);
  ...
  for (idx_t source = 0; source < vertex_count; source++) {
    ...
    if (state.vertex_mask.empty() && !filter_label) {
      out_degrees[source] = state.snapshot->outgoing_offsets[source + 1] -
                            state.snapshot->outgoing_offsets[source];
      continue;
    }
    for (idx_t offset = state.snapshot->outgoing_offsets[source];
      ...
      out_degrees[source] +=
          ...
          (!filter_label ||
           state.snapshot->outgoing_label_ids[offset] == required_label);
    }
  }
  ...
  for (idx_t iteration = 1; iteration <= data.max_iterations; iteration++) {
    ...
    for (idx_t source = 0; source < vertex_count; source++) {
      if (InVertexProjection(state.vertex_mask, source) &&
          out_degrees[source] == 0) {
        dangling_rank += state.ranks[source];
      }
    }
    auto base =
        (1.0 - data.damping) / static_cast<double>(projected_count) +
    ...
    for (idx_t source = 0; source < vertex_count; source++) {
      ...
      auto contribution = data.damping * state.ranks[source] /
                          static_cast<double>(out_degrees[source]);
      for (idx_t offset = state.snapshot->outgoing_offsets[source];
           offset < state.snapshot->outgoing_offsets[source + 1]; offset++) {
        if (filter_label &&
            state.snapshot->outgoing_label_ids[offset] != required_label) {
...
static void PageRankFunction(ClientContext &context, TableFunctionInput &input,
                             DataChunk &output) {
...
static unique_ptr<GraphAlgorithmBindData>
ReadGraphAlgorithmBind(const TableFunctionBindInput &input,
...
static unique_ptr<FunctionData> ComponentBind(const string &algorithm_name,
                                              TableFunctionBindInput &input,
                                              vector<LogicalType> &return_types,
                                              vector<string> &names) {
...
static unique_ptr<FunctionData> WccBind(ClientContext &,
                                        TableFunctionBindInput &input,
                                        vector<LogicalType> &return_types,
                                        vector<string> &names) {
...
static unique_ptr<FunctionData> SccBind(ClientContext &,
                                        TableFunctionBindInput &input,
                                        vector<LogicalType> &return_types,
                                        vector<string> &names) {
...
static unique_ptr<GlobalTableFunctionState>
ComponentInit(ClientContext &, TableFunctionInitInput &) {
...
static void ComputeWcc(ClientContext &context,
                       const GraphAlgorithmBindData &data,
  ...
  for (idx_t source = 0; source < vertex_count; source++) {
    ...
    VisitRange(
        state.snapshot->outgoing_offsets, state.snapshot->outgoing_neighbors,
        state.snapshot->outgoing_edge_ids, state.snapshot->outgoing_label_ids,
        source, filter_label, required_label, [&](idx_t target, uint64_t) {
          if (!InVertexProjection(state.vertex_mask, target)) {
            return true;
          }
  ...
  vector<uint64_t> minimum_ids(vertex_count,
  ...
  for (idx_t vertex = 0; vertex < vertex_count; vertex++) {
    ...
    minimum_ids[root] = MinValue<uint64_t>(minimum_ids[root],
                                           state.snapshot->vertex_ids[vertex]);
...
static void ComputeScc(ClientContext &context,
                       const GraphAlgorithmBindData &data,
                       ComponentState &state) {
  state.snapshot = GqlGetCsrSnapshot(context, data.graph_name);
  const auto vertex_count = state.snapshot->vertex_ids.size();
  ...
  for (idx_t start = 0; start < vertex_count; start++) {
    ...
    visited[start] = true;
    dfs_stack.push_back({start, state.snapshot->outgoing_offsets[start],
                         state.snapshot->outgoing_offsets[start + 1]});
    while (!dfs_stack.empty()) {
      ...
      while (frame.offset < frame.end) {
        ...
        if (filter_label &&
            state.snapshot->outgoing_label_ids[edge_offset] != required_label) {
          continue;
        }
        ...
        dfs_stack.push_back({neighbor,
                             state.snapshot->outgoing_offsets[neighbor],
  ...
  for (auto order = finish_order.rbegin(); order != finish_order.rend();
    ...
    while (!stack.empty()) {
      ...
      minimum_id =
          MinValue<uint64_t>(minimum_id, state.snapshot->vertex_ids[vertex]);
      VisitRange(
          state.snapshot->incoming_offsets, state.snapshot->incoming_neighbors,
          state.snapshot->incoming_edge_ids, state.snapshot->incoming_label_ids,
          vertex, filter_label, required_label, [&](idx_t neighbor, uint64_t) {
            if (InVertexProjection(state.vertex_mask, neighbor) &&
                !visited[neighbor]) {
              visited[neighbor] = true;
...
static void WriteComponentRows(ComponentState &state, DataChunk &output) {
  ...
  while (count < STANDARD_VECTOR_SIZE &&
         state.offset < state.component_ids.size()) {
    auto vertex = state.offset++;
    if (!InVertexProjection(state.vertex_mask, vertex)) {
      continue;
    }
...
static void WccFunction(ClientContext &context, TableFunctionInput &input,
                        DataChunk &output) {
...
static void SccFunction(ClientContext &context, TableFunctionInput &input,
                        DataChunk &output) {
  auto &data = input.bind_data->Cast<GraphAlgorithmBindData>();
  auto &state = input.global_state->Cast<ComponentState>();
  if (!state.initialized) {
...
TriangleCountBind(ClientContext &, TableFunctionBindInput &input,
                  vector<LogicalType> &return_types, vector<string> &names) {
  auto result = ReadGraphAlgorithmBind(input, "triangle count");
  names = {"vertex_id", "triangle_count", "degree",
           "local_clustering_coefficient", "global_triangle_count"};
  return_types = {LogicalType::UBIGINT, LogicalType::UBIGINT,
...
static unique_ptr<GlobalTableFunctionState>
TriangleCountInit(ClientContext &, TableFunctionInitInput &) {
  return make_uniq<TriangleCountState>();
}

static void ComputeTriangleCounts(ClientContext &context,
                                  const GraphAlgorithmBindData &data,
  ...
  for (idx_t source = 0; source < vertex_count; source++) {
    ...
    for (idx_t offset = state.snapshot->outgoing_offsets[source];
      ...
      undirected_edges.emplace_back(MinValue<idx_t>(source, target),
                                    MaxValue<idx_t>(source, target));
    }
  }
  ...
  for (const auto &edge : undirected_edges) {
    auto source =
        is_lower_rank(edge.first, edge.second) ? edge.first : edge.second;
    forward_degrees[source]++;
  }
  vector<idx_t> forward_offsets(vertex_count + 1, 0);
  for (idx_t vertex = 0; vertex < vertex_count; vertex++) {
    forward_offsets[vertex + 1] =
        forward_offsets[vertex] + forward_degrees[vertex];
  }
  ...
  for (idx_t source = 0; source < vertex_count; source++) {
    ...
    for (idx_t offset = forward_offsets[source];
         offset < forward_offsets[source + 1]; offset++) {
      markers[forward_neighbors[offset]] = source;
    }
    for (idx_t offset = forward_offsets[source];
         offset < forward_offsets[source + 1]; offset++) {
      auto middle = forward_neighbors[offset];
      for (idx_t middle_offset = forward_offsets[middle];
           middle_offset < forward_offsets[middle + 1]; middle_offset++) {
...
static void TriangleCountFunction(ClientContext &context,
  ...
  idx_t count = 0;
  while (count < STANDARD_VECTOR_SIZE &&
         state.offset < state.triangle_counts.size()) {
    auto vertex = state.offset++;
    if (!InVertexProjection(state.vertex_mask, vertex)) {
      continue;
    }
    ...
    coefficients[count] =
        state.degrees[vertex] < 2
            ? 0.0
            : (2.0 * state.triangle_counts[vertex]) /
                  (static_cast<double>(state.degrees[vertex]) *
                   (state.degrees[vertex] - 1));
...
static unique_ptr<FunctionData> LccBind(ClientContext &,
                                        TableFunctionBindInput &input,
                                        vector<LogicalType> &return_types,
                                        vector<string> &names) {
  auto result = ReadGraphAlgorithmBind(input, "local clustering coefficient");
  names = {"vertex_id", "degree", "directed_neighbor_edge_count",
...
static unique_ptr<GlobalTableFunctionState> LccInit(ClientContext &,
                                                    TableFunctionInitInput &) {
  return make_uniq<LccState>();
}

static void ComputeLcc(ClientContext &context,
                       const GraphAlgorithmBindData &data, LccState &state) {
  state.snapshot = GqlGetCsrSnapshot(context, data.graph_name);
  const auto vertex_count = state.snapshot->vertex_ids.size();
  idx_t projected_count;
  state.vertex_mask =
  ...
  for (const auto &edge : directed_edges) {
    undirected_edges.emplace_back(MinValue(edge.first, edge.second),
                                  MaxValue(edge.first, edge.second));
  }
  std::sort(undirected_edges.begin(), undirected_edges.end());
  undirected_edges.erase(
      std::unique(undirected_edges.begin(), undirected_edges.end()),
  ...
  for (idx_t vertex = 0; vertex < vertex_count; vertex++) {
    neighbor_offsets[vertex + 1] =
        neighbor_offsets[vertex] + state.degrees[vertex];
  }
  ...
  undirected_edges.shrink_to_fit();
  for (idx_t vertex = 0; vertex < vertex_count; vertex++) {
    std::sort(neighbors.begin() + neighbor_offsets[vertex],
              neighbors.begin() + neighbor_offsets[vertex + 1]);
  }
...
static void LccFunction(ClientContext &context, TableFunctionInput &input,
                        DataChunk &output) {
  auto &data = input.bind_data->Cast<GraphAlgorithmBindData>();
...
static unique_ptr<FunctionData> DegreeBind(ClientContext &,
                                           TableFunctionBindInput &input,
                                           vector<LogicalType> &return_types,
                                           vector<string> &names) {
...
static unique_ptr<GlobalTableFunctionState>
DegreeInit(ClientContext &, TableFunctionInitInput &) {
  return make_uniq<DegreeState>();
}

static void ComputeDegrees(ClientContext &context,
                           const GraphAlgorithmBindData &data,
                           DegreeState &state) {
  state.snapshot = GqlGetCsrSnapshot(context, data.graph_name);
  ...
  for (idx_t vertex = 0; vertex < vertex_count; vertex++) {
    ...
    if (!filter_label && state.vertex_mask.empty()) {
      state.out_degrees[vertex] = state.snapshot->outgoing_offsets[vertex + 1] -
                                  state.snapshot->outgoing_offsets[vertex];
      state.in_degrees[vertex] = state.snapshot->incoming_offsets[vertex + 1] -
                                 state.snapshot->incoming_offsets[vertex];
      continue;
    }
    for (idx_t offset = state.snapshot->outgoing_offsets[vertex];
      ...
      state.out_degrees[vertex] +=
          InVertexProjection(state.vertex_mask,
                             state.snapshot->outgoing_neighbors[offset]) &&
          (!filter_label ||
           state.snapshot->outgoing_label_ids[offset] == required_label);
    }
    for (idx_t offset = state.snapshot->incoming_offsets[vertex];
      ...
      state.in_degrees[vertex] +=
          InVertexProjection(state.vertex_mask,
                             state.snapshot->incoming_neighbors[offset]) &&
          (!filter_label ||
           state.snapshot->incoming_label_ids[offset] == required_label);
    }
  }
  state.initialized = true;
}

static void DegreeFunction(ClientContext &context, TableFunctionInput &input,
                           DataChunk &output) {
  auto &data = input.bind_data->Cast<GraphAlgorithmBindData>();
  ...
  idx_t count = 0;
  while (count < STANDARD_VECTOR_SIZE &&
         state.offset < state.out_degrees.size()) {
    auto vertex = state.offset++;
    if (!InVertexProjection(state.vertex_mask, vertex)) {
      continue;
    }
...
static unique_ptr<FunctionData> ClosenessBind(ClientContext &,
                                              TableFunctionBindInput &input,
                                              vector<LogicalType> &return_types,
                                              vector<string> &names) {
  if ((input.inputs.size() != 1 && input.inputs.size() != 2) ||
  ...
  result->edge_label = ReadLabelParameter(input, "edge_label");
  result->vertex_label = ReadVertexLabel(input, 1);
  names = {"vertex_id", "reachable_count", "distance_sum",
...
static unique_ptr<GlobalTableFunctionState>
ClosenessInit(ClientContext &, TableFunctionInitInput &) {
  return make_uniq<ClosenessState>();
}

static void ComputeCloseness(ClientContext &context,
                             const ClosenessBindData &data,
                             ClosenessState &state) {
  state.snapshot = GqlGetCsrSnapshot(context, data.graph_name);
  auto vertex_count = state.snapshot->vertex_ids.size();
  idx_t projected_count;
  state.vertex_mask =
  ...
  for (idx_t source = 0; source < vertex_count; source++) {
    ...
    while (head < queue.size()) {
      ...
      VisitNeighbors(
          *state.snapshot, vertex, data.direction, filter_label, required_label,
          [&](idx_t neighbor, uint64_t) {
            ...
            if (distance_sum >
                NumericLimits<uint64_t>::Maximum() - distances[neighbor]) {
    ...
    if (distance_sum > 0) {
      ...
      state.scores[source] =
          (reachable * reachable) / (static_cast<double>(projected_count - 1) *
...
static void ClosenessFunction(ClientContext &context, TableFunctionInput &input,
                              DataChunk &output) {
...
AlgorithmCallBind(ClientContext &, TableFunctionBindInput &input,
                  vector<LogicalType> &return_types, vector<string> &names) {
  if (input.inputs.size() != 4 || input.inputs[1].IsNull() ||
  ...
  if (values.size() < required_configuration_count ||
      values.size() > expected_configuration_count ||
      types.size() != values.size() ||
      input.input_table_types.size() !=
          (definition->input_mode == GqlProcedureInputMode::NONE
               ? 1
               : expected_input_count)) {
    throw BinderException("Invalid input shape for GQL procedure 'algo.%s'",
                          procedure_name);
  }
  ...
  for (const auto &argument : definition->arguments) {
    ...
    if (configuration_index >= values.size()) {
      ...
      throw BinderException("Missing configuration for GQL procedure 'algo.%s'",
                            procedure_name);
    }
    ...
    if (literal_type == GqlLiteralType::NULL_VALUE ||
        (argument.type.id == GqlTypeId::STRING &&
         literal_type != GqlLiteralType::STRING)) {
      throw BinderException("Invalid configuration for GQL procedure 'algo.%s'",
                            procedure_name);
    }
  ...
  for (const auto &output : definition->outputs) {
    ...
    return_types.push_back(output.type.id == GqlTypeId::INTEGER
                               ? LogicalType::UBIGINT
...
static unique_ptr<GlobalTableFunctionState>
AlgorithmCallGlobalInit(ClientContext &, TableFunctionInitInput &) {
  return make_uniq<AlgorithmCallGlobalState>();
}

static unique_ptr<LocalTableFunctionState>
AlgorithmCallLocalInit(ExecutionContext &, TableFunctionInitInput &,
                       GlobalTableFunctionState *) {
  return make_uniq<AlgorithmCallLocalState>();
}

static OperatorResultType AlgorithmCallInput(ExecutionContext &,
                                             TableFunctionInput &input,
                                             DataChunk &child,
                                             DataChunk &output) {
  ...
  if (data.definition->input_mode != GqlProcedureInputMode::BATCH ||
      child.ColumnCount() != 1) {
  ...
  for (idx_t row = 0; row < child.size(); row++) {
    ...
    state.frontier.push_back(
        value.DefaultCastAs(LogicalType::UBIGINT).GetValue<uint64_t>());
  }
  return OperatorResultType::NEED_MORE_INPUT;
}

static void InitializePipelineBfs(ClientContext &context,
                                  const AlgorithmCallBindData &data,
  ...
  state.frontier.erase(
      std::unique(state.frontier.begin(), state.frontier.end()),
      state.frontier.end());
  auto traversal = make_uniq<TraversalBindData>();
  traversal->graph_name = graph_name;
  traversal->vertex_label =
      data.configuration.size() > 1 ? data.configuration[1].value : string();
  auto bfs = make_uniq<BfsState>();
  bfs->initialized = true;
  bfs->snapshot = std::move(snapshot);
  idx_t projected_count;
  bfs->vertex_mask =
      BuildVertexMask(*bfs->snapshot, traversal->vertex_label, projected_count);
  bfs->visited.resize(bfs->snapshot->vertex_ids.size(), false);
  for (auto vertex_id : state.frontier) {
    auto vertex = RequireProjectedVertex(*bfs->snapshot, bfs->vertex_mask,
                                         vertex_id, "frontier");
...
static void InitializePipelineDfs(ClientContext &context,
                                  const AlgorithmCallBindData &data,
                                  AlgorithmCallLocalState &state) {
  auto graph_name = data.configuration[0].value;
  ...
  for (auto vertex_id : state.frontier) {
    dfs->frontier.push_back(RequireProjectedVertex(
        *dfs->snapshot, dfs->vertex_mask, vertex_id, "frontier"));
  }
  state.nested_global_state = std::move(dfs);
}

static void PipelineDfsFunction(ClientContext &context, PipelineDfsState &state,
                                DataChunk &output) {
  TraversalOutputWriter writer(output);
  idx_t count = 0;
  while (count < STANDARD_VECTOR_SIZE) {
    ...
    if (state.stack.empty()) {
      while (state.next_frontier < state.frontier.size() &&
             state.visited[state.frontier[state.next_frontier]]) {
        state.next_frontier++;
      }
      ...
      state.visited[start] = true;
      writer.Write(count++, *state.snapshot, {start, 0, 0, 0, false},
                   state.visit_order++);
      state.stack.push_back(
          MakeDfsFrame(*state.snapshot, start, 0, CsrDirection::OUT));
      continue;
    }
    auto &frame = state.stack.back();
    while (true) {
      idx_t neighbor;
      uint64_t edge_id;
      if (!NextDfsNeighbor(*state.snapshot, CsrDirection::OUT, false, 0,
      ...
      TraversalRow row{neighbor, frame.depth + 1, frame.vertex, edge_id, true};
      writer.Write(count++, *state.snapshot, row, state.visit_order++);
      state.stack.push_back(MakeDfsFrame(*state.snapshot, neighbor, row.depth,
                                         CsrDirection::OUT));
      break;
    }
  }
...
static void InitializeAlgorithmCall(ExecutionContext &context,
                                    const AlgorithmCallBindData &data,
                                    AlgorithmCallLocalState &state) {
  ...
  if (name == "bfs" || name == "sssp") {
    ...
    if (name == "sssp" &&
        state.nested_global_state->Cast<BfsState>().queue.size() != 1) {
      throw InvalidInputException(
          "GQL SSSP requires exactly one distinct source vertex");
    }
  } else if (name == "dfs") {
...
AlgorithmCallFinalize(ExecutionContext &context, TableFunctionInput &input,
                      DataChunk &output) {
  auto &data = input.bind_data->Cast<AlgorithmCallBindData>();
  auto &state = input.local_state->Cast<AlgorithmCallLocalState>();
  if (!state.initialized) {
  ...
  if (data.definition->name == "bfs" || data.definition->name == "sssp") {
    TableFunctionInput nested(state.nested_bind_data.get(), nullptr,
                              state.nested_global_state.get());
    BfsFunction(context.client, nested, output);
  } else if (data.definition->name == "dfs") {
    PipelineDfsFunction(context.client,
                        state.nested_global_state->Cast<PipelineDfsState>(),
                        output);
  } else {
    TableFunctionInput nested(state.nested_bind_data.get(), nullptr,
                              state.nested_global_state.get());
  ...
  return output.size() == 0 ? OperatorFinalizeResultType::FINISHED
...
TableFunction GqlAlgorithmCallFunction() {
  TableFunction function("gql_algorithm_call",
                         ...
                         nullptr, AlgorithmCallBind, AlgorithmCallGlobalInit,
...
TableFunction GqlBfsFunction() {
  TableFunction function("bfs", {LogicalType::VARCHAR, LogicalType::BIGINT},
                         BfsFunction);
...
TableFunction GqlDfsFunction() {
  TableFunction function("dfs", {LogicalType::VARCHAR, LogicalType::BIGINT},
                         DfsFunction);
  function.bind = TraversalBind;
...
TableFunction GqlSsspFunction() {
  TableFunction function("sssp", {LogicalType::VARCHAR, LogicalType::BIGINT},
                         BfsFunction);
...
TableFunction GqlTriangleCountFunction() {
  TableFunction function("triangle_count", {LogicalType::VARCHAR},
...
TableFunction GqlClosenessFunction() {
  TableFunction function("closeness", {LogicalType::VARCHAR},
                         ClosenessFunction);
