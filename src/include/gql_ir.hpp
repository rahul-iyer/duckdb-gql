...
enum class GqlLogicalOperatorType : uint8_t { UNIT, MATCH, FILTER, INNER_APPLY, LEFT_APPLY, CALL, PROJECT };
...
public:
	explicit GqlLogicalFilter(shared_ptr<GqlBoundExpression> predicate_p)
	    : GqlLogicalOperator(GqlLogicalOperatorType::FILTER), predicate(std::move(predicate_p)) {
	}
...
public:
	GqlLogicalInnerApply() : GqlLogicalOperator(GqlLogicalOperatorType::INNER_APPLY) {
	}
...
public:
	explicit GqlLogicalLeftApply(idx_t optional_stage_p)
	    : GqlLogicalOperator(GqlLogicalOperatorType::LEFT_APPLY), optional_stage(optional_stage_p) {
	}
